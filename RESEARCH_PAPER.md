...
We present HaHaScore, a multimodal system for predicting humor strength (0–100) at the sentence level from text and audio. Through systematic ablation, we find that **text-only models achieve AUC ~0.50 (indistinguishable from random)** for sentence-level humor detection, while **audio prosodic features provide the primary discriminative signal**. We show that **temporal arc modeling** — processing audio as a sequence of segments with a bidirectional GRU — achieves AUC 0.842. Our **Cascade Gate fusion** mechanism, where text confidence modulates audio contribution, achieves **AUC 0.860** (+0.002 over cross-attention, +0.018 over audio-only), and **AUC 0.590 on real human laughter labels** — confirming that our models capture perceived funniness (delivery), not behavioral laughter response. Self-training on pseudo-labels does not improve results, confirming that the pseudo-label quality ceiling has been reached. This challenges the prevailing text-first approach in humor detection and establishes audio delivery as the dominant factor in sentence-level humor perception.
