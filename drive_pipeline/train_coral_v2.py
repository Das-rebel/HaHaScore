...
============================
Multi-task (humor + laughter) + CORAL covariance alignment.
Uses pre-computed gold_laughter.npy from real CSVs.
"""
import os
...
def load_standup_data():
    ...
    # Load gold laughter from pre-computed file
...
class MultiTaskV8(nn.Module):
    def __init__(self, base_v8_state):
        ...
        v8_state = {k.replace('v8.', ''): v for k, v in base_v8_state.items() if k.startswith('v8.')}
    ...
    def forward(self, text, audio, cross):
        ...
        cross_h = self.backbone.cross_proj(
            cross if cross is not None else torch.zeros(audio.shape[0], audio.shape[1], 16, device=audio.device)
        )
        ...
        temporal_output = self.backbone.advanced_biGRU(fused)
...
def train_coral(args):
    print("=" * 60)
    print("🎯 Multi-Task + CORAL Domain Adaptation v2")
    print("=" * 60)
    ...
    for epoch in range(args.epochs):
        ...
        for batch_start in range(0, n_use - args.batch_size, args.batch_size):
            ...
            # CORAL: flatten features and split into source/target halves
    ...
    # Convert continuous gold to binary (above/below median)
    if (y_gold > 0).any():
    ...
    metrics = {
        ...
        'laugh_auc': float(laugh_auc),
        'config': vars(args),
        'note': 'Multi-task + CORAL domain adaptation v2',
    }
...
def main():
    ...
    p.add_argument('--output-name', default='v8_coral_v2')
