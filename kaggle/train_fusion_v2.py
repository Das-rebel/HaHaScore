# v2 Fusion: RoBERTa text + WavLM audio -> laughter prediction
# Data: 48 videos, 467K word-level samples, 84K positive
# Kaggle: GPU T4x2, dataset subhajitdas/chuckle-fusion-48v
...
AUD = '/kaggle/input/chuckle-fusion-48v/fusion_audio'
LAB = '/kaggle/input/chuckle-fusion-48v/fusion_aligned_labels.jsonl'
SR = 16000
WIN = 1.5  # seconds of audio context around word

print(f"Device: {DEVICE}")

# ---------- Data ----------
rows = [json.loads(l) for l in open(LAB)]
# group by video, keep rows with timestamps
by_vid = {}
for r in rows:
...
# audio files present
audio_files = {f.rsplit('.',1)[0]: os.path.join(AUD, f) for f in os.listdir(AUD)}
...
class FusionDS(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if wave is None:
            ...
            self.cache[vid] = wave
        c = int(SR * WIN / 2)
        s = max(0, int(r['start']*SR) - c); e = min(len(wave), int(r['end']*SR) + c)
        seg = wave[s:e]
...
class Fusion(nn.Module):
    def __init__(self):
        ...
        self.fuse = nn.Sequential(
            nn.Linear(768+768, 256), nn.ReLU(), nn.Dropout(0.3),
            nn.Linear(256, 64), nn.ReLU(), nn.Dropout(0.2),
            nn.Linear(64, 1))
    def forward(self, ids, mask, wav):
        t = self.txt(ids, attention_mask=mask).pooler_output
        a = self.aud(wav).extract_features.mean(1)  # (B, 768)
...
def run(rows, epochs=3, bs=16):
    ...
    for ep in range(epochs):
        ...
        for ids, mask, wav, y in dl:
            opt.zero_grad()
            loss = crit(model(ids.to(DEVICE), mask.to(DEVICE), wav.to(DEVICE)),
                        y.float().to(DEVICE))
...
run(tr_r + va_r, epochs=1)
f1, auc = evaluate(FusionDS(te_r, audio_files))
print(f"TEST: F1 {f1:.4f} AUC {auc:.4f}")
torch.save(model.state_dict(), 'fusion_v2_48v.pt')
print("saved fusion_v2_48v.pt")
